"""
CIFAR-10 specific data partitioning and loading utilities.
"""
import numpy as np
...
from .partition_utils import DatasetContainer, partition_data_dirichlet, partition_data_pathological, partition_test_data_proportional
...
def _create_client_dataloader_cifar10(args_tuple):
...
def load_and_partition_cifar10(data_dir, partition_method, n_clients, partition_alpha, batch_size, logger, max_workers=None):
    ...
    """
    logger.info("--------- Loading and Partitioning CIFAR-10 Data ---------")
    ...
    elif partition_method.lower() == 'patho':
        # For pathological partition, partition_alpha represents classes_per_client
        classes_per_client = int(partition_alpha) if partition_alpha >= 1 else 2
        logger.info(f"Partitioning training data via Pathological (classes_per_client={classes_per_client})")
        net_dataidx_map, traindata_cls_counts = partition_data_pathological(y_train, n_clients, classes_per_client)
    else:
    ...
    if max_workers is not None:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            results = list(tqdm(executor.map(_create_client_dataloader_cifar10, task_args), total=n_clients, desc="Creating Client DataLoaders"))
    else:
    ...
    dataset_container.class_num = len(np.unique(y_train))  # 10 for CIFAR-10
    logger.info("--------- CIFAR-10 Data Loading and Partitioning Complete ---------")
